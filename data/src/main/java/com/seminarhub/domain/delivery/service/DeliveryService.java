...
public class DeliveryService {
    ...
    public List<Delivery> findNextBatch(LocalDateTime startAt, LocalDateTime endAt, long lastId, int pageSize) {
        return repository.findByDeliveryStatusNotAndInstDtBetweenAndIdGreaterThan(
                DeliveryStatus.DELIVERED, startAt, endAt, lastId, PageRequest.of(0, pageSize));
    }
    @Transactional public List<Delivery> saveAll(List<Delivery> deliveries) {
        return repository.saveAll(deliveries);
    }
    @Transactional public void updateStatusById(Long id, DeliveryStatus status) {
        repository.findById(id).ifPresent(delivery -> delivery.updateStatus(status));
    }
